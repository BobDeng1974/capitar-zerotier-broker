...
Result::
	`name`::: The name supplied in the call.
...
The intention here is that the REST server will usethe above parameters
to construct an otpauth:// URL that can be turned into a QR code for
import into authentication apps.  The REST server should construct
the label as `$controller:$user`.
